...
namespace signlang::ai_chat {
  ...
  auto IpcAppStateMonitor::create_node() -> iox2::Node<iox2::ServiceType::Ipc> {
    ...
    if (!node.has_value()) {
      throw std::runtime_error("Failed to create iceoryx2 node for ai_chat state monitor");
    }
  ...
  auto IpcAppStateMonitor::create_event_service(const iox2::Node<iox2::ServiceType::Ipc>& node,
    ...
    if (!service.has_value()) {
      throw std::runtime_error("Failed to open or create iceoryx2 app state event service in ai_chat: " + service_name);
    }
  ...
  auto IpcAppStateMonitor::create_listener(const StateEventService& service) -> iox2::Listener<iox2::ServiceType::Ipc> {
    ...
    if (!listener.has_value()) {
      throw std::runtime_error("Failed to create iceoryx2 app state listener in ai_chat");
    }
  ...
  auto IpcAppStateMonitor::create_blackboard_service(const iox2::Node<iox2::ServiceType::Ipc>& node,
    ...
    if (!opened_service.has_value()) {
      throw std::runtime_error("Failed to open iceoryx2 app state blackboard service in ai_chat: " + service_name);
    }
  ...
  auto IpcAppStateMonitor::create_reader(const StateBlackboardService& service)
    ...
    if (!reader.has_value()) {
      throw std::runtime_error("Failed to create iceoryx2 app state blackboard reader in ai_chat");
    }
  ...
  auto IpcAppStateMonitor::create_state_entry(iox2::Reader<iox2::ServiceType::Ipc, state_machine::AppStateKey>& reader)
    ...
    if (!entry.has_value()) {
      throw std::runtime_error("Failed to create iceoryx2 app state blackboard entry reader in ai_chat");
    }
  ...
  auto IpcSignlangResultSubscriber::create_node() -> iox2::Node<iox2::ServiceType::Ipc> {
    ...
    auto node =
        iox2::NodeBuilder().signal_handling_mode(iox2::SignalHandlingMode::Disabled).create<iox2::ServiceType::Ipc>();
    if (!node.has_value()) {
      throw std::runtime_error("Failed to create iceoryx2 node for ai_chat signlang result subscriber");
    }
    return std::move(node.value());
  }
  ...
  auto IpcSignlangResultSubscriber::create_subscriber(const iox2::Node<iox2::ServiceType::Ipc>& node,
    ...
    auto service = node.service_builder(signlang::common::ipc::service_name_from_string(service_name))
                       ...
                       .open_or_create();
    if (!service.has_value()) {
      throw std::runtime_error("Failed to open signlang result service in ai_chat: " + service_name);
    }

    auto subscriber = service.value().subscriber_builder().buffer_size(buffer_size).create();
    if (!subscriber.has_value()) {
      throw std::runtime_error("Failed to create ai_chat signlang result subscriber");
    }
    return std::move(subscriber.value());
  }

  IpcAiChatResultPublisher::IpcAiChatResultPublisher(const std::string& service_name,
                                                     std::uint64_t publisher_history_size) :
      node_{create_node()}, publisher_{create_publisher(node_, service_name, publisher_history_size)} {}

  auto IpcAiChatResultPublisher::create_node() -> iox2::Node<iox2::ServiceType::Ipc> {
    iox2::set_log_level_from_env_or(iox2::LogLevel::Warn);

    auto node =
        iox2::NodeBuilder().signal_handling_mode(iox2::SignalHandlingMode::Disabled).create<iox2::ServiceType::Ipc>();
    if (!node.has_value()) {
      throw std::runtime_error("Failed to create iceoryx2 node for ai_chat result publisher");
    }
    return std::move(node.value());
  }

  auto IpcAiChatResultPublisher::create_publisher(const iox2::Node<iox2::ServiceType::Ipc>& node,
                                                  const std::string& service_name, std::uint64_t history_size)
      -> iox2::Publisher<iox2::ServiceType::Ipc, AiChatResult, void> {
    auto service = node.service_builder(signlang::common::ipc::service_name_from_string(service_name))
                       .publish_subscribe<AiChatResult>()
                       .history_size(history_size)
                       .open_or_create();
    if (!service.has_value()) {
      throw std::runtime_error("Failed to open ai_chat result service: " + service_name);
    }

    auto publisher = service.value().publisher_builder().create();
    if (!publisher.has_value()) {
      throw std::runtime_error("Failed to create ai_chat result publisher");
    }
    return std::move(publisher.value());
  }

  void IpcAiChatResultPublisher::publish(const AiChatResult& result) {
    const auto send_result = publisher_.send_copy(result);
    if (!send_result.has_value()) {
      throw std::runtime_error("Failed to publish ai_chat result through iceoryx2");
    }
  }
